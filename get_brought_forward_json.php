...
$data = []; // สร้างอาร์เรย์เก็บข้อมูล
$sum = 0;   // ตัวแปรรวมค่า totalAmountNew
        ...
        do {
            if ($result = $conn->store_result()) {
                while ($row = $result->fetch_assoc()) {
                    $row['totalAmountNew'] = floatval($row['totalAmountNew']); // แปลงเป็นตัวเลข
                    $row['rate'] = floatval($row['rate']); // แปลงเป็นตัวเลข
                    $row['first_adjusted_balance'] = $row['first_adjusted_balance'] !== null ? floatval($row['first_adjusted_balance']) : null;
